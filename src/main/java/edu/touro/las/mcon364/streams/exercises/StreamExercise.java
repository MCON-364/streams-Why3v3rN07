...
public class StreamExercise {
    ...
    public List<Integer> getStudentGrades(String studentName) {
        return Optional.ofNullable(gradebook.get(studentName)).orElse(new ArrayList<>());
    }
    // =========================================================================
    ...
    public double calculateAverage(String studentName) {
        return Optional.ofNullable(gradebook.get(studentName)).orElse(List.of()).stream().mapToInt(Integer::intValue).average().orElse(0.0);
    }
    ...
    public List<Integer> getAllGradesFlattened() {
        return gradebook.values().stream().flatMap(Collection::stream).sorted().toList();
    }
    ...
    public int findHighestGrade() {
        return this.getAllGradesFlattened().stream().mapToInt(Integer::intValue).max().orElse(0);
    }
    ...
    public int findLowestGrade() {
        return this.getAllGradesFlattened().stream().mapToInt(Integer::intValue).min().orElse(0);
    }
    ...
    public long getTotalGradeCount() {
        return this.getAllGradesFlattened().stream().count();
    }
    ...
    public List<String> getPassingStudents(double threshold) {
        return gradebook.keySet().stream().filter(n -> calculateAverage(n) >= threshold).toList();
    }
    ...
    public Map<String, List<String>> groupByPerformance() {
        return gradebook.keySet().stream().collect(Collectors.groupingBy(entry -> getLetterGrade(calculateAverage(entry))));
    }
    ...
    public Map<String, Double> getStudentAverages() {
        return gradebook.keySet().stream().collect(Collectors.toMap(name -> name, name -> calculateAverage(name)));
    }
    ...
    public String findTopPerformer() {
        return gradebook.entrySet().stream().max(Comparator.comparingDouble(entry -> calculateAverage(entry.getKey()))).map(Map.Entry::getKey).orElse(null);
    }
    ...
    public List<String> getStudentsWithPerfectScore() {
        return gradebook.entrySet().stream().filter(n -> n.getValue().stream().anyMatch(score -> score == 100)).map(n -> n.getKey()).toList();
    }
    ...
    public double calculateClassAverage() {
        return this.getAllGradesFlattened().stream().mapToInt(n -> n).average().orElse(0.0);
    }
